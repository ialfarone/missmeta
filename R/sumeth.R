...
#'
#' @param eff1 Numeric vector. Estimates for the first effect size (e.g., "EstCR").
#' @param eff2 Numeric vector. Estimates for the second effect size (e.g., "EstSR").
#' @param se1 Numeric vector. Standard errors associated with `eff1`.
#' @param se2 Numeric vector. Standard errors associated with `eff2`.
#' @param cov12 Numeric vector. Covariances between the two effect sizes.
#' @param method Character string. Name of the distribution used for imputing data.
#'
#' @return An object of class \code{sumeth}, which contains the pooled results.
#' @export
#'
...
sumeth <- function(eff1, eff2, se1, se2, cov12, method) {
  ...
  U_list <- lapply(seq_len(m), function(i) {
