...
```{r meta}
outls <- lapply(out, function(x) x$imputations)
outls <- unlist(outls, recursive = FALSE)
...
res <- lapply(outls, function(data) {
  ...
  Sigma_list <- lapply(seq_len(nrow(data)), function(i) {
    matrix(
      c(
        data$SECR[i]^2,
...
df_all <- do.call(rbind, lapply(sumres, function(x) {
  out <- x$results
  out$method <- x$method
  out
}))
...
df2_all <- do.call(rbind, lapply(sumres2, function(x) {
  data.frame(
    outcome  = names(x$Q_bar),
    estimate = x$Q_bar,
    se       = x$se,
    ci_lb    = x$ci[,1],
    ci_ub    = x$ci[,2],
